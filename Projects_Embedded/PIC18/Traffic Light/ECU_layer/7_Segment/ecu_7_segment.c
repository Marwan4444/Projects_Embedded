...
STD_ReturnType segment_write_num(const segment_t *_segment, uint8 _number){
    ...
    if(_segment == NULL || _number > 9) ret = E_NOT_OK;
        ...
        gpio_pin_write_logic(&(_segment->segment_pins[1]), ((_number >> 1) & 0x01));
        gpio_pin_write_logic(&(_segment->segment_pins[2]), ((_number >> 2) & 0x01));
        gpio_pin_write_logic(&(_segment->segment_pins[3]), ((_number >> 3) & 0x01));
    }
    return ret;
}